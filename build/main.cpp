...
int main( int argc, char** argv )
{
	...
	printf( "--------------------------------------------------------\n" );
	Mat lImg = imread( lFn, 1 );
	Mat rImg = imread( rFn, 1 );
	...
	Mat l,r;
	lDis.convertTo(l,CV_16UC1);
	rDis.convertTo(r,CV_16UC1);
	imwrite( lDisFn, l );
	imwrite( rDisFn, r );
	smPyr[ 0 ]->PostProcess( ppMtd );
	Mat pl,pr;
	lDis.convertTo(pl,CV_16UC1);
	rDis.convertTo(pr,CV_16UC1);
	imwrite("l_chk.png",pl);
	imwrite("r_chk.png",pr);
#ifdef _DEBUG
